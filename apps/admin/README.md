...
Source creation and re-ingestion use idempotency keys. If a network or server failure leaves an operation's outcome unknown, retrying the identical operation reuses its original key, including after dismissing and reopening the re-ingestion dialog. An explicit authoritative refresh reconciles the source before a later re-ingestion receives a new key; changing any creation field likewise starts a new logical operation with a new key. The interface reports whether creation or re-ingestion queued a new job or reused an existing canonical source or active job. Disabling retrieval preserves indexed content but excludes it from answers; enabling restores the currently committed representation without forcing re-ingestion.
