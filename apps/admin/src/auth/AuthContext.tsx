...
export function AuthProvider({
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((error) => {
        ...
        setUser(null);
        setState(
          error instanceof AdminApiError && error.kind === 'unauthenticated'
            ? 'unauthenticated'
            : 'error',
        );
      });
  ...
  const logout = useCallback(async () => {
    try {
      ...
      if (!(error instanceof AdminApiError && error.kind === 'unauthenticated')) throw error;
    } finally {
      sessionExpired();
    }
  }, [api, sessionExpired]);
