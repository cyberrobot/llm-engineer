...
function ConfirmationDialog({ confirmation, pending, error, onClose }: { confirmation: Confirmation; pending: boolean; error?: string; onClose(): void }) {
  ...
  return <dialog ref={dialogRef} aria-labelledby="operation-confirm-title" onCancel={(event) => { event.preventDefault(); if (!pending) onClose(); }}>
...
function JobTable({ page }: { page:OperationalJobs }) { return <div className="table-wrap"><table><thead><tr><th>ID</th><th>Type</th><th>Status</th><th>Created</th><th>Duration</th><th>Retries</th><th>Node</th><th>Last error</th></tr></thead><tbody>{page.items.map((job)=><tr key={job.id}><td data-label="Job ID"><Link to={`/admin/operations/jobs/${job.id}`}>{job.id}</Link></td><td data-label="Type">{job.jobType}</td><td data-label="Status"><Status value={job.status}/></td><td data-label="Created">{dateTime(job.createdAt)}</td><td data-label="Duration">{duration(job.durationMs)}</td><td data-label="Retries">{job.retryCount}</td><td data-label="Node">{job.executionNode??'Not available'}</td><td data-label="Last error">{job.lastError??'None'}</td></tr>)}</tbody></table></div>; }
