...
def test_administrator_knowledge_sources_ignore_public_lifecycle(monkeypatch, status, visibility):
    ...
    try:
        ...
        changed = assistant.deactivate(at=now + timedelta(seconds=1)).change_visibility(
            AssistantVisibility.private, at=now + timedelta(seconds=1)
        )
        assistant_repository.update(changed, expected_updated_at=assistant.updated_at)
        with get_connection() as connection:
            before = connection.execute(
                "SELECT knowledge_sources.assistant_id, documents.retrieval_state, "
                "document_ingestion_jobs.status FROM knowledge_sources "
        ...
        assert before == (str(assistant.id), "disabled", "queued")
        assert client.get(base).json()["total"] == 1
        assert client.get(f"{base}/{source_id}").json()["retrieval_state"] == "disabled"

        restored = changed.activate(at=now + timedelta(seconds=2)).change_visibility(
            AssistantVisibility.public, at=now + timedelta(seconds=2)
        )
        assistant_repository.update(restored, expected_updated_at=changed.updated_at)
        with get_connection() as connection:
        ...
        if document_id is not None:
            with get_connection() as connection:
                ...
                connection.execute("DELETE FROM documents WHERE id=%s", (document_id,))
        with get_connection() as connection:
            connection.execute("DELETE FROM assistants WHERE id=%s", (str(assistant.id),))
