...
from assistant.domain.citation_verifier import verify_sentence
...
def test_supported_sentence_returns_best_source_id_and_score():
    ...
    with patch(
        "assistant.domain.citation_verifier.get_embedding", return_value=[1, 0]
    ) as get_embedding:
...
def test_empty_sentence_returns_unsupported_without_embedding():
    ...
    with patch("assistant.domain.citation_verifier.get_embedding") as get_embedding:
...
def test_fallback_embedding_is_used_when_chunk_embedding_is_missing():
    ...
    with patch(
        "assistant.domain.citation_verifier.get_embedding",
        side_effect=[[1, 0], [1, 0]],
    ) as get_embedding:
        result = verify_sentence("Staff must wear surgical scrubs.", chunks)
